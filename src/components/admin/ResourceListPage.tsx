        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-xs">
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-gray-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-gray-900/5"
              value={query}
          ...
          <div className="space-y-3 sm:hidden">
            {filteredData.length === 0 ? (
              <p className="text-center text-sm text-gray-500">
                Aucun élément trouvé. Essayez un autre filtre ou créez un nouvel enregistrement.
              </p>
            ) : (
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((row, idx) => (
                    <tr key={row.id ?? idx} className="hover:bg-gray-50/60">
                      {columns.map((col) => (
                        <td key={col.key} className="whitespace-nowrap px-3 py-3 text-gray-700">
                          {col.render ? col.render(row) : String(row[col.key])}
                        </td>
                      ))}
