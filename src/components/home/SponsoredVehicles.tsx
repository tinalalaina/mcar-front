...
export const SponsoredVehicles = () => {
  ...
  return (
    <AnimatedSection className="pb-2 pt-1" delay={0}>
      <div className="mb-5">
        <span className="inline-flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-violet-700/90">
          <span className="h-[3px] w-10 rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-400" />
          Sponsorisées
        </span>

        <h2 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-[1.8rem]">
          Les annonces sponsorisées mises en avant sur la plateforme
        </h2>

        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 sm:text-[15px]">
          Retrouvez les véhicules mis en avant pour offrir plus de visibilité à
          certaines annonces et faciliter leur découverte.
        </p>
      </div>
          ...
          <CarouselContent className="-ml-3 md:-ml-4">
            ...
            {!isLoading &&
              vehicles.map((vehicle: any, index: number) => {
                ...
                const isCurrentlyReserved = toBoolean(vehicle.is_currently_reserved);
                const isReservable =
                  typeof vehicle.is_reservable === "boolean"
