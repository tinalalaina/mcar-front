...
import { useNewVehicles } from "@/useQuery/vehiculeStatsUseQuery";
...
export const NewVehicles = () => {
  const { data: vehicles = [], isLoading } = useNewVehicles();
  ...
  return (
    <AnimatedSection className="pb-2 pt-1" delay={0}>
      <div className="mb-5">
        <span className="inline-flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-emerald-600/90">
        ...
        <h2 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-[1.8rem]">
          Les dernières voitures publiées à découvrir
        </h2>

        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 sm:text-[15px]">
          Retrouvez les annonces les plus récentes, déjà visibles sur la
          plateforme et prêtes à être consultées.
        </p>
      </div>
                    ...
                    <AnimatedItem delay={index * 35}>
                      ...
                      >
                        <VehicleCard
                          ...
                          popular={Boolean((vehicle as any).is_popular) || trips >= 3}
