...
import { Loader2, MessageSquare, Plus, ShieldCheck } from "lucide-react";
...
export default function MyTickets() {
  ...
  const stats = useMemo(() => {
    return [
      ...
      {
        label: "Liés à une réservation",
        value: myTickets.filter((t) => !!t.reservation).length,
        valueClassName: "text-blue-700",
      },
      {
        label: "Liés à un véhicule",
        value: myTickets.filter((t) => !!t.vehicule).length,
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100/40 p-4 md:p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
          <div className="h-1.5 bg-gradient-to-r from-slate-950 via-slate-800 to-slate-700" />
          ...
          <div className="flex flex-col gap-6 p-6 md:flex-row md:items-center md:justify-between md:p-8">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-600 md:text-base">
                Gérez vos échanges avec le support concernant vos véhicules,
                réservations et demandes générales.
              </p>
            </div>
        ...
        {filteredTickets.length === 0 ? (
          <SupportEmptyState
            ...
            description="Modifiez vos filtres ou créez un nouveau ticket."
          />
        ) : (
          <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
            {paginatedTickets.map((ticket) => (
              <TicketCard
                key={ticket.id}
                ticket={ticket}
                href={`/prestataire/supports/ticket/${ticket.id}`}
